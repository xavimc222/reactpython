export const documentation = `# OpenSearch Query Summary
...
**Goal**: Group data by one variable (\`location\`)
 and calculate a metric (\`sum\`)
 on a second variable (\`onHandQuantityValue\`).
**Index**: \`ai-research-shell-inference-3\`
\`\`\`python
import logging
import json
